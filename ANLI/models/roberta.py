...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        ...
        # x = self.dropout(x)
        if self.training:
            if self.dp_mask1 is None:
                self.dp_mask1 = torch.zeros_like(x).bernoulli_(1 - self.dp_prob) / (1 - self.dp_prob)
            x = self.dp_mask1 * x
        ...
        # x = self.dropout(x)
        if self.training:
            if self.dp_mask2 is None:
                self.dp_mask2 = torch.zeros_like(x).bernoulli_(1 - self.dp_prob) / (1 - self.dp_prob)
            x = self.dp_mask2 * x
        x = self.out_proj(x)
        return x
