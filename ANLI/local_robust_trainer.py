...
class Trainer:
    ...
    mi_estimator: Optional[InfoNCE] = None
    mi_upper_estimator: Optional[CLUBv2] = None
    ...
    def _save(self, output_dir: Optional[str] = None):
        ...
        # save MI estimator
        if self.mi_estimator:
            mi_estimator = self.mi_estimator.module if hasattr(self.mi_estimator, "module") else self.mi_estimator
        ...
        torch.save(self.args, os.path.join(output_dir, "training_args.bin"))
        torch.save(self.eval_hist, os.path.join(output_dir, "eval_hist.bin"))
    ...
    def evaluate_mi(
        ...
        output = self._prediction_mi_loop(eval_dataloader, description="Evaluation")
    ...
    def _prediction_mi_loop(
        ...
        if self.args.local_rank != -1:
            ...
            if self.mi_upper_estimator:
                mi_info = self.distributed_concat(mi_info, num_total_examples=self.num_examples(dataloader))
            if self.mi_estimator:
                mi_info_lower = self.distributed_concat(mi_info_lower, num_total_examples=self.num_examples(dataloader))
                mi_info_nonrobust_lower = self.distributed_concat(mi_info_nonrobust_lower, num_total_examples=self.num_examples(dataloader))
