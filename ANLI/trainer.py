...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.mi_estimator:
                    full_loss, loss_dict = self._training_step(model, inputs, optimizer)
                    tr_loss += full_loss
                else:
    ...
    def _train_mi_estimator(self, outputs, inputs=None):
        ...
        elif self.mi_estimator.version == 1:
            return self.mi_estimator.update(embedding_layer, last_hidden)
        elif self.mi_estimator.version == 2:
            return self.mi_estimator.update(embedding_layer, sentence_embedding)
        elif self.mi_estimator.version == 3:
            embeddings = []
            lengths = self.get_seq_len(inputs)
            for i, length in enumerate(lengths):
                embeddings.append(embedding_layer[i, :length])
            embeddings = torch.cat(embeddings)  # [-1, 768]
            return self.mi_estimator.update(embedding_layer, embeddings, self.global_step)
    ...
    def _eval_mi_estimator(self, outputs, inputs=None):
        ...
        if self.mi_estimator.version == 0:
            ...
            return self.mi_estimator.mi_est(embedding_layer, last_hidden)
        elif self.mi_estimator.version == 2:
            return self.mi_estimator.mi_est(embedding_layer, sentence_embedding)
        elif self.mi_estimator.version == 3:
            estimates = []
            lengths = self.get_seq_len(inputs)
            for i, length in enumerate(lengths):
                estimates.append(self.mi_estimator.mi_est(embedding_layer, embedding_layer[i, :length]))
            return torch.tensor(estimates).mean()
    ...
    def _training_step(
        ...
        if self.mi_estimator:
            upper_bound = self._train_mi_estimator(outputs, inputs)
            loss += upper_bound
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            with torch.no_grad():
                outputs = model(**inputs)
                if self.mi_estimator:
                    mi_info += self._eval_mi_estimator(outputs, inputs),
                if has_labels:
